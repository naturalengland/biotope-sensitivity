...
folder <- "F:/derived_data/Sensitivity_per_pressure/output/sbgr_biotp_eunis_match/"
...
sbgr.matched.btpt.w.rpl <- results.files %>% 
       plyr::llply(function(x){ 
               ...
               sbgr.dfs.lst.by.h.lvl.tmp <- x %>% # select a dataframe (temporary name for dataframe where PER sbgr dataframe is stored, and run through the rest of the code)
                       plyr::dlply("h.lvl") # splits it into a list of dataframes accoprding to the biotope classiciation level


                # the dataframes now need to be comapred, and use level 6 values, and only replace NA values using level 5 and 4, but not replace EUNIs codes provided for eunis codes.
                
                l6 <- sbgr.dfs.lst.by.h.lvl.tmp[[3]]#"3" should be replaced by something to make it eunis level 6 category, which is currently [[3]]
                l5 <- sbgr.dfs.lst.by.h.lvl.tmp[[2]] # eunis level 5
                l4 <- sbgr.dfs.lst.by.h.lvl.tmp[[1]] # eunis level 4
        ...
        #saveRDS(sbgr.matched.btpt.w.rpl, paste0("./sbgr_matched_btpt_w_rpl.rds"))
