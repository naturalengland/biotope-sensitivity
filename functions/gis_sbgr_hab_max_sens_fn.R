...
act.sbgr.bps.gis <- sbgr.BAP.max.sens %>%
        llply(function(x){ # split the list into its dataframes, and provide a list at the end (containing dataframes) (which are eventaully piped onto bind_cols into a single dataframe /preferred over ldply as this would cause row bind, not column bind.
                ...
                #        dplyr::select(-(2:5)) %>% # remove not immediately relevant columns
                #        dplyr::group_by(eval(group.by)) %>% # group the results by a unique identifier for polygon: "pkey"
                #        dplyr::summarise_all(max) %>% # summarise (obtain the maximum) ALL the remaining columns (according to the grouping); this was key to ensuring that dynamic names operate; if columns 2:5 were needed that could be bound after the fact, as these variables cannot be summarised as they are not numerical
                #        dplyr::select(-(`eval(group.by)`)) #remove the surpuflous variable created from using parse/eval functions.
                #carry on: 
                sbgr.hab.gis.2  <-  sbgr.hab.gis %>%
                        dplyr::ungroup() %>%
